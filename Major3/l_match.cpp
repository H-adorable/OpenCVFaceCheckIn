...
int FindMatches(Image im1, Keypoint keys1, Image im2, Keypoint keys2);
...
*/
int match (std::vector<std::string> argv)
{
//    int arg = 0;
...
//      FatalError("Command line does not specify all images and keys.");
    ...
    char* c1 = new char[argv[0].length() + 1];
    strcpy(c1, argv[0].c_str());
    im1 = ReadPGMFile(c1);
    delete [] c1;

    char* c2 = new char[argv[1].length() + 1];
    strcpy(c2, argv[1].c_str());
    im2 = ReadPGMFile(c2);
    delete [] c2;

    char* c3 = new char[argv[2].length() + 1];
    strcpy(c3, argv[2].c_str());
    k1 = ReadKeyFile(c3);
    delete [] c3;

    char* c4 = new char[argv[3].length() + 1];
    strcpy(c4, argv[3].c_str());
    k2 = ReadKeyFile(c4);
    delete [] c4;

    int c = FindMatches(im1, k1, im2, k2);
...
//    WritePGM(stdout, result);
    ...
    FILE *out = NULL;
    out = fopen ("D:/qt/test/build-LoweSIFT-Qt_5_4_1_with_MinGW491_32-Debug/data/out.pgm", "w");
    WritePGM(out, result);
...
Image CombineImagesVertically(Image im1, Image im2)
{
    ...
    cols = std::max(im1->cols, im2->cols);
